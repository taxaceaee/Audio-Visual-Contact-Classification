...
def main() -> None:
    ...
    if cand["backbone"] != "efficientnet_b3" or lock.get("test_loaded"):
    ...
    X_hand = np.load(OUT / "image_timm_features/efficientnet_b3.ra2_in1k/hand_train_full/X.npy")
    y_hand = hand.y.to_numpy(dtype=np.int64)
    if not np.array_equal(y_hand, np.load(OUT / "image_timm_features/efficientnet_b3.ra2_in1k/hand_train_full/y.npy").astype(np.int64)):
        raise AssertionError("hand alignment")
    model = Pipeline([("scale", StandardScaler()), ("model", LogisticRegression(C=0.1, class_weight="balanced", max_iter=1200, random_state=42))]).fit(X_hand, y_hand)

    # Open robot/test only after the hand grouped selection lock.
    ...
    X_test = np.load(OUT / "image_timm_features/efficientnet_b3.ra2_in1k/robot_test/X.npy")
    y_test = test.y.to_numpy(dtype=np.int64)
    if not np.array_equal(y_test, np.load(OUT / "image_timm_features/efficientnet_b3.ra2_in1k/robot_test/y.npy").astype(np.int64)):
        raise AssertionError("test image alignment")
    raw_img = model.predict_proba(X_test)
    ...
    high_path = OUT / "audio_feature_benchmarks/audio_highsr_temporal_tta_select/reports/audio_highsr_temporal_tta_select_final_test_predictions.csv"
    pair_path = OUT / "audio_feature_benchmarks/audio_group_consistency_pair_blend_select/reports/audio_group_consistency_pair_blend_select_final_test_predictions.csv"
    high, pair = pd.read_csv(high_path), pd.read_csv(pair_path)
    for name, df in [("high", high), ("pair", pair)]:
        if not np.array_equal(df.audio_file.astype(str).to_numpy(), raw_test.audio_file.astype(str).to_numpy()):
            raise AssertionError(f"audio {name} alignment")
    p_audio_window = suite.normalize(0.8 * high[suite.PROBA_COLUMNS].to_numpy(dtype=np.float64) + 0.2 * pair[suite.PROBA_COLUMNS].to_numpy(dtype=np.float64))
    p_audio = suite.segment_lift(test.audio_file, p_audio_window)
    ...
    result = {
        ...
        "audio_pipeline": "0.8 highsr_tta + 0.2 pairwise, segment_lift",
    ...
    (REPORT / "new_timm_final_test_metrics.json").write_text(json.dumps(result, indent=2, default=float), encoding="utf-8")
    pd.DataFrame([{k: v for k, v in result.items() if not isinstance(v, (dict, list))}]).to_csv(REPORT / "new_timm_final_test_report.csv", index=False)
    pd.DataFrame(result["confusion_matrix_4class"], index=NAMES, columns=NAMES).to_csv(REPORT / "new_timm_final_test_confusion_matrix_4class.csv")
    pd.DataFrame(result["confusion_matrix_binary"], index=["ambient", "noambient"], columns=["ambient", "noambient"]).to_csv(REPORT / "new_timm_final_test_confusion_matrix_binary.csv")
