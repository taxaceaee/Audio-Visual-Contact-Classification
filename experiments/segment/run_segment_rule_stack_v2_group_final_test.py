"""Robot/test once for retuned hier + meta rule stack v2."""
...
def main():
    ...
    # Persist locked base segment outputs for downstream, hand-selected rule decoders.
    p_h = np.zeros_like(p_meta)
    p_h[:, 0] = 1.0 - contact
    p_h[:, 1:] = contact[:, None] * mat
    np.savez(
        OUT / "segment_rule_stack_v2_base_segment_outputs.npz",
        segment_ids=np.asarray(tu),
        row_segment_ids=np.asarray(sk),
        specimen_ids=np.asarray(sel.spec(pd.Series(tu)).to_numpy()),
        pred_h=np.asarray(pred_h),
        pred_m=np.asarray(pred_m),
        p_h=np.asarray(p_h),
        p_meta=np.asarray(p_meta),
    )
    by = (yt > 0).astype(int)
    ...
    (OUT / "segment_rule_stack_v2_final_test_metrics.json").write_text(
