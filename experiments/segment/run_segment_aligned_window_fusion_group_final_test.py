...
def main():
    ...
    hfr=suite.load_manifest(ROOT/'audio_visual_dataset_default/dataset.csv',ROOT/'audio_visual_dataset_default','hand_train'); yh=hfr.y.to_numpy(np.int64); xh=np.load(IMG/'hand_train_full/X.npy').astype(np.float32); model=make_pipeline(StandardScaler(),LogisticRegression(C=.03,max_iter=1000,class_weight='balanced',random_state=42)); model.fit(xh[yh>0],yh[yh>0]-1)
    tfr=suite.load_manifest(ROOT/'audio_visual_dataset_robo_default/dataset.csv',ROOT/'audio_visual_dataset_robo_default','robot_test'); yt=tfr.y.to_numpy(np.int64); xt=np.load(IMG/'robot_test/X.npy').astype(np.float32); tu,tc,_,_=pool(tfr.audio_file,xt,yt); row_img=model.predict_proba(xt)
    audio=pd.read_csv('outputs/audio_feature_benchmarks/audio_lift_source_blend_select/reports/audio_lift_source_blend_select_final_test_predictions.csv'); raw=pd.read_csv(ROOT/'audio_visual_dataset_robo_default/dataset.csv'); assert np.array_equal(audio.audio_file.astype(str).to_numpy(),raw.audio_file.astype(str).to_numpy()); proba=suite.normalize(audio[suite.PROBA_COLUMNS].to_numpy(np.float64)); pc,lg=avr.avr_inputs(proba); row_audio=suite.normalize(np.exp(lg)); sk=seg(tfr.audio_file).to_numpy(); seg_pc=np.array([pc[sk==k].mean() for k in tu]); row_mat=suite.normalize(row_audio*(1-w)+row_img*w); seg_mat=np.vstack([suite.normalize(row_mat[sk==k].mean(0,keepdims=True))[0] for k in tu]); pred_seg=np.where(seg_pc>=th,seg_mat.argmax(1)+1,0); order={k:i for i,k in enumerate(tu)}; pred=np.array([pred_seg[order[k]] for k in sk]); by=(yt>0).astype(int); bp=(pred>0).astype(int)
    ...
    (OUT/'segment_aligned_window_final_test_metrics.json').write_text(json.dumps(result,indent=2,default=float)); print(json.dumps(result,indent=2,default=float))
