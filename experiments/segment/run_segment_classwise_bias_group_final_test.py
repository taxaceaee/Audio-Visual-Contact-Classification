...
def main():
    lock=json.loads((OUT/'selection_lock.json').read_text()); assert not lock.get('test_loaded'); b=lock['selected_candidate']; w=np.array([float(b['leaf_weight']),float(b['trunk_weight']),float(b['twig_weight'])]); bias=np.array([1.0,float(b['trunk_bias']),float(b['twig_bias'])]); th=float(b['contact_threshold']); audio_base.configure_feature_set('total240')
    hfr=suite.load_manifest(ROOT/'audio_visual_dataset_default/dataset.csv',ROOT/'audio_visual_dataset_default','hand_train'); yh=hfr.y.to_numpy(np.int64); xh=np.load(IMG/'hand_train_full/X.npy').astype(np.float32); _,hc,hx,hy=pool(hfr.audio_file,xh,yh); model=make_pipeline(StandardScaler(),LogisticRegression(C=.03,max_iter=1000,class_weight='balanced',random_state=42)); model.fit(hx[hy>0],hy[hy>0]-1)
    ...
    (OUT/'segment_classwise_bias_final_test_metrics.json').write_text(json.dumps(result,indent=2,default=float)); print(json.dumps(result,indent=2,default=float))
