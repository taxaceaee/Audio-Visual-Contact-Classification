...
def features(a,i,pc):
    return np.hstack([np.log(np.clip(a,1e-6,1)),np.log(np.clip(i,1e-6,1)),a,i,pc[:,None],a.max(1,keepdims=True),i.max(1,keepdims=True)]).astype(np.float32)
def main():
    ...
    # Meta-C is selected only by a second group CV over already OOF base probabilities.
    z=features(sa,sio,spc); np.save(OUT/'hand_meta_oof_features.npy',z); np.save(OUT/'hand_meta_oof_y.npy',sy); meta_rows=[]
    for cval in [.01,.03,.1,.3,1.0,3.0]:
        fs=[]
        for tr,va in folds:
            m=make_pipeline(StandardScaler(),LogisticRegression(C=cval,max_iter=2000,class_weight='balanced',multi_class='multinomial',random_state=42)); m.fit(z[tr],sy[tr]); fs.append(f1_score(sy[va],m.predict(z[va]),average='macro',zero_division=0))
        meta_rows.append({'C':cval,'mean_cv_macro_f1':float(np.mean(fs)),'worst_cv_macro_f1':float(np.min(fs))})
    board=pd.DataFrame(meta_rows).sort_values(['mean_cv_macro_f1','worst_cv_macro_f1'],ascending=False).reset_index(drop=True); board.to_csv(OUT/'hand_meta_calibration_leaderboard.csv',index=False); best=board.iloc[0].to_dict(); lock={'protocol':'segment_oof_probability_meta_calibration_group_OOF','selection_data':'hand/default only','group_column':'specimen_group','audio_source':'locked group-aware OOF audio','image_source':'CLIP mean embedding per segment OOF probabilities','image_role':'material only via base image model; meta fusion only','test_loaded':False,'selected_candidate':best}; (OUT/'selection_lock.json').write_text(json.dumps(lock,indent=2,default=float)); print(json.dumps(lock,indent=2,default=float))
