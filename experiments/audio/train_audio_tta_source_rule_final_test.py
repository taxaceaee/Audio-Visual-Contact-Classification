...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Apply a train-only selection rule to the original audio TTA leaderboard, "
            "then load robot/test only after the rule lock is written."
        )
    )
    ...
    parser.add_argument(
        "--rule",
        choices=[
            "best_hier_within_margin",
            "top_hier",
            "best_hgb_clean_robot",
        ],
        default="best_hier_within_margin",
    )
...
def select_row(leaderboard: pd.DataFrame, args: argparse.Namespace) -> dict:
    ...
    if args.rule == "best_hier_within_margin":
        eligible = leaderboard[
            (leaderboard["selection_score"] >= top_score - args.margin)
            & leaderboard["base_candidate"].astype(str).str.contains("hier", regex=False)
        ].copy()
        if eligible.empty:
            raise RuntimeError("No hierarchical row is within the requested margin")
        row = eligible.sort_values(
            ["selection_score", "tta_contact_macro_f1", "tta_macro_f1"],
            ascending=False,
        ).iloc[0]
    elif args.rule == "top_hier":
        eligible = leaderboard[leaderboard["base_candidate"].astype(str).str.contains("hier", regex=False)].copy()
        row = eligible.sort_values(
            ["selection_score", "tta_contact_macro_f1", "tta_macro_f1"],
            ascending=False,
        ).iloc[0]
    elif args.rule == "best_hgb_clean_robot":
        eligible = leaderboard[
            leaderboard["base_candidate"].astype(str).str.contains("hgb_default", regex=False)
            & leaderboard["tta_recipe"].astype(str).isin(["clean_half_aug_half", "all_equal"])
        ].copy()
        row = eligible.sort_values(
            ["worst_single_view_hybrid", "selection_score", "tta_macro_f1"],
            ascending=False,
        ).iloc[0]
    else:
...
def main() -> None:
    ...
    source_leaderboard = args.source_run / "reports" / "audio_tta_contact_stress_cv_select_oof_tta_leaderboard.csv"
    if not source_leaderboard.exists():
        raise FileNotFoundError(f"Missing OOF leaderboard: {source_leaderboard}")

    run_slug = f"audio_tta_source_rule_{args.rule}"
    ...
    final_row.update(
        {
            "selected_by": f"hand_default_audio_only_tta_source_rule_{args.rule}",
    ...
    joblib.dump(
        {
            "protocol": "audio_only_tta_source_locked_rule_no_test_until_lock",
    ...
    print("\nFinal robot/test result after frozen source-rule TTA selection:")
    print(
