...
OUT = Path('outputs')
REPORT = OUT / 'audio_feature_benchmarks/audio_only_locked_clip_bias_group_selection'
...
def main():
    ...
    pa = suite.normalize(audio[suite.PROBA_COLUMNS].to_numpy(dtype=np.float64))
    iw = float(cand['image_weight'])
    bias = np.array([0.0, 0.0, float(cand['trunk_bias']), float(cand['twig_bias'])])
    p = suite.normalize(np.exp((1-iw) * np.log(pa) + iw * np.log(pi) + bias[None, :]))
    pred = p.argmax(axis=1)
    by, bp = (y > 0).astype(int), (pred > 0).astype(int)
    result = {
        'split': 'robot_test_final',
        'protocol': 'audio_only_locked_clip_bias_after_5fold_specimen_group_selection',
        'n': int(len(y)),
    ...
    (REPORT / 'audio_only_locked_clip_bias_final_test_metrics.json').write_text(json.dumps(result, indent=2, default=float))
