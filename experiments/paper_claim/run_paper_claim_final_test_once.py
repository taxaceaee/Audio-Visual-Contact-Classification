#!/usr/bin/env python3
"""PAPER CLAIM — pure single-shot robot open (step 2/2).

Requirements:
  - selection_lock.json exists (hand-only)
  - claim not already sealed
  - recipe hash matches RECIPE.json
  - opens robot exactly once, writes CLAIM_SEAL.json
...
def main():
    ...
    # protect-trunk material bias (or identity)
    if mode != "none":
        soft = p_meta_t.copy()
        h = p_h_t.argmax(1)
        soft[h == 2] = p_h_t[h == 2]
