"""One-shot robot: locked 0.837 contact + hand-selected gated material rescue."""
...
def main():
    ...
    modes = [tuple(x) for x in lock["selected_modes"]]
    src = sc["source"]
    conf_th = float(sc["conf_th"])
    margin_th = float(sc["margin_th"])
    T = float(sc["T"])
    ...
    experts = {
        "multi": fit_multi(multi_h[contact_tr], ymat, 0.05),
        "clip": fit_multi(hx["clip"][contact_tr], ymat, 0.08),
        "clap": fit_multi(clap_hs[contact_tr], ymat, 0.05),
        "av": fit_multi(
            np.hstack([ha_s["clean"], hw_s, multi_h, clap_hs])[contact_tr], ymat, 0.03
        ),
    }
    ...
    if sc["mode"] == "none" or not modes:
        pred_final = pred
    else:
        mat = {
            "multi": proba_aligned(experts["multi"], multi_t),
            "clip": proba_aligned(experts["clip"], tx["clip"]),
            "clap": proba_aligned(experts["clap"], clap_ts),
            "av": proba_aligned(
                experts["av"], np.hstack([ta_s, tw_s, multi_t, clap_ts])
            ),
            "meta3": proto.normalize(p_meta_t[:, 1:4]),
            "hier3": proto.normalize(p_h_t[:, 1:4]),
        }
        mat["blend_mh"] = proto.normalize(0.5 * mat["meta3"] + 0.5 * mat["hier3"])
        mat["blend_img_av"] = proto.normalize(0.5 * mat["multi"] + 0.5 * mat["av"])
        mat["blend_all"] = proto.normalize(
            (mat["multi"] + mat["av"] + mat["meta3"] + mat["clap"]) / 4.0
        )
        mat3 = mat.get(src, mat["blend_mh"])
        pred_final = apply_gate(pred, mat3, conf_th, margin_th, T, bl, bt, bg, modes)
    ...
    result = {
        ...
        **proto.metrics_bundle(yt, pred_rows),
        "invariants": {
            ...
            "robot_uda": False,
            "gated_material_rescue": True,
            "locked_0837_contact": True,
        },
    }
    proto.write_final_metrics(
        OUT / "multimodal_085_gated_mat_rescue_final_test_metrics.json", result
    )
