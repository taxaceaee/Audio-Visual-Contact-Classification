...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Image-only deep transfer learning. Pretrained image backbones extract "
            "deep features from hand/default only, train/val selects classifier and "
            "bias, writes a selection lock, then robot/test is loaded for one final "
            "evaluation."
        )
    )
    parser.add_argument("--root", type=Path, default=None)
    parser.add_argument("--output", type=Path, default=Path("outputs"))
    parser.add_argument("--run-slug", default="image_deep_transfer_specimen_select")
    ...
    parser.add_argument(
        ...
        help="Comma-separated torchvision backbones: resnet18,resnet34,resnet50,vit_b16.",
    )
...
def manifest_signature(frame: pd.DataFrame, backbone: str, image_size: int) -> str:
    digest = hashlib.sha256()
    digest.update(f"backbone={backbone}|image_size={image_size}|imagenet_norm\n".encode("utf-8"))
    for row in frame[["image_path", "y"]].itertuples(index=False):
        digest.update(f"{row.image_path}|{int(row.y)}\n".encode("utf-8"))
    return digest.hexdigest()


def cache_paths(feature_cache_dir: Path, backbone: str, image_size: int, split_name: str) -> dict[str, Path]:
    split_dir = feature_cache_dir / f"{backbone}_{image_size}" / split_name
...
def make_backbone(backbone: str) -> tuple[nn.Module, int]:
...
def extract_deep_features(
    ...
    paths = cache_paths(feature_cache_dir, backbone_name, image_size, split_name)
    signature = manifest_signature(frame, backbone_name, image_size)
    ...
    if ready and not force_rebuild:
        ...
        if (
            metadata.get("feature_family") == "torchvision_deep_image_embedding"
            and metadata.get("backbone") == backbone_name
            and int(metadata.get("image_size", -1)) == image_size
            ...
            print(f"Loaded deep cache {backbone_name}/{split_name}: {payload['X'].shape} in {elapsed:.3f}s", flush=True)
            return payload, {
                "cache_hit": True,
    ...
    model, feature_dim = make_backbone(backbone_name)
    model.eval().to(device)
    dataset = ImageManifestDataset(frame, make_transform(image_size))
    loader = DataLoader(
        dataset,
    ...
    if X.shape != (len(frame), feature_dim):
        raise AssertionError(f"{backbone_name}/{split_name}: expected {(len(frame), feature_dim)}, got {X.shape}")
    payload = {
        "X": X,
    ...
    metadata = {
        "feature_family": "torchvision_deep_image_embedding",
    ...
    print(
        f"Saved deep cache {backbone_name}/{split_name}: {payload['X'].shape} "
...
def make_candidates(backbone: str, random_state: int) -> dict[str, CandidateSpec]:
    ...
    specs = [
        ...
        CandidateSpec(f"{backbone}_logreg_C3_balanced", backbone, lambda: logreg(3.0, "balanced")),
...
def evaluate_candidate(
    ...
    if hasattr(model, "predict_proba"):
        ...
        bias_items = [("none", np.zeros(4, dtype=np.float64), default_pred), ("tuned", tuned_bias, img.predict_with_bias(val_proba, tuned_bias))]
        predict_time = time.perf_counter() - start
    else:
        pred = model.predict(X_val)
        bias_items = [("none", np.zeros(4, dtype=np.float64), pred)]
        predict_time = time.perf_counter() - start
    ...
    for bias_mode, bias, pred in bias_items:
        row = img.make_report_row(
            ...
            feature_name=f"{spec.backbone}_deep_embedding",
...
def main() -> None:
    ...
    print("Protocol  = image-only deep transfer selection; robot/test after lock", flush=True)
    ...
    for backbone in backbones:
        payload, timing = extract_deep_features(
            train_df,
            "hand_train_full",
            backbone,
            args.image_size,
    ...
    split_summary = {
        "protocol": "image_only_deep_transfer_train_val_selection_robot_after_lock",
    ...
    test_payload, test_timing = extract_deep_features(
        test_df,
        "robot_test",
    ...
    final_row = img.make_report_row(
        ...
        feature_name=f"{selected_spec.backbone}_deep_embedding",
    ...
    final_row.update(
        {
            "selected_by": "hand_train_val_image_only_deep_transfer",
    ...
    joblib.dump(
        {
            "protocol": "image_only_deep_transfer_no_test_until_lock",
    ...
    print("\nFinal robot/test result after frozen image-only deep-transfer selection:", flush=True)
    print(
