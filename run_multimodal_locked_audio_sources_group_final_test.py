...
def main() -> None:
    ...
    if cand["audio_candidate"] != "report_gate_proba+report_gate_onehot@0.5":
    ...
    # Test is opened only after the hand-only lock above.
    ...
    path = OUT / "audio_feature_benchmarks/audio_report_grade_gate_select/reports/audio_report_grade_gate_select_final_test_predictions.csv"
    ...
    p_gate = suite.normalize(audio[suite.PROBA_COLUMNS].to_numpy(dtype=np.float64))
    p_onehot = np.zeros_like(p_gate)
    p_onehot[np.arange(len(audio)), audio.pred_y.to_numpy(dtype=np.int64)] = 1.0
    p_audio = suite.normalize(0.5 * p_gate + 0.5 * p_onehot)
    ...
    (REPORT / "locked_audio_sources_final_test_metrics.json").write_text(json.dumps(result, indent=2, default=float), encoding="utf-8")
    pd.DataFrame([{k: v for k, v in result.items() if not isinstance(v, (dict, list))}]).to_csv(REPORT / "locked_audio_sources_final_test_report.csv", index=False)
    pd.DataFrame(result["confusion_matrix_4class"], index=NAMES, columns=NAMES).to_csv(REPORT / "locked_audio_sources_final_test_confusion_matrix_4class.csv")
    pd.DataFrame(result["confusion_matrix_binary"], index=["ambient", "noambient"], columns=["ambient", "noambient"]).to_csv(REPORT / "locked_audio_sources_final_test_confusion_matrix_binary.csv")
