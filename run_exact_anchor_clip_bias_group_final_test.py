...
def main() -> None:
    ...
    # Open robot/test only after the hand-only selection lock.
    ...
    for col, cls in enumerate(model.classes_): p_img[:, int(cls)] = raw_img[:, col]
    p_img = suite.normalize(p_img)
    ...
    p = suite.normalize(np.exp((1-image_weight)*np.log(suite.normalize(p_audio)) + image_weight*np.log(suite.normalize(p_img)) + bias[None, :]))
    pred = p.argmax(1)
    by, bp = (y_test > 0).astype(int), (pred > 0).astype(int)
    result = {
        "split": "robot_test_final", "protocol": "locked_after_5fold_specimen_group_hand_only_bias_selection", "n": int(len(y_test)), "locked_candidate": cand,
    ...
    (REPORT / "exact_anchor_clip_bias_final_test_metrics.json").write_text(json.dumps(result, indent=2, default=float), encoding="utf-8")
    pd.DataFrame([{k:v for k,v in result.items() if not isinstance(v,(dict,list))}]).to_csv(REPORT / "exact_anchor_clip_bias_final_test_report.csv", index=False)
    pd.DataFrame(result["confusion_matrix_4class"], index=NAMES, columns=NAMES).to_csv(REPORT / "exact_anchor_clip_bias_final_test_confusion_matrix_4class.csv")
    pd.DataFrame(result["confusion_matrix_binary"], index=["ambient","noambient"], columns=["ambient","noambient"]).to_csv(REPORT / "exact_anchor_clip_bias_final_test_confusion_matrix_binary.csv")
