...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Audio-only rich handcrafted features with contact-aware stress-CV. "
            "Uses only hand/default for selection; robot/test is loaded after lock."
        )
    )
    ...
    parser.add_argument(
        "--rich-feature-cache-dir",
        type=Path,
        default=Path("outputs/audio_feature_benchmarks/audio_rich_stress_cv_select/features"),
    )
...
def make_rich_contact_candidates() -> dict[str, stress.StressCandidate]:
    specs = [
        stress.StressCandidate("rich_contact_hgb_default__clean", "direct_hgb_default", "single", ("clean",)),
        stress.StressCandidate("rich_contact_hgb_default__all_aug", "direct_hgb_default", "single", STRESS_VIEWS),
        stress.StressCandidate("rich_contact_hgb_regularized__clean", "direct_hgb_regularized", "single", ("clean",)),
        stress.StressCandidate("rich_contact_hgb_regularized__all_aug", "direct_hgb_regularized", "single", STRESS_VIEWS),
        stress.StressCandidate("rich_contact_lgbm__clean", "direct_lightgbm_regularized", "single", ("clean",)),
        stress.StressCandidate("rich_contact_lgbm__all_aug", "direct_lightgbm_regularized", "single", STRESS_VIEWS),
        stress.StressCandidate("rich_contact_hier_extra_hgb__clean", "hier_extra_hgb", "single", ("clean",)),
        stress.StressCandidate("rich_contact_hier_extra_hgb__all_aug", "hier_extra_hgb", "single", STRESS_VIEWS),
        stress.StressCandidate("rich_contact_hier_hgb_lgbm__all_aug", "hier_hgb_lightgbm", "single", STRESS_VIEWS),
    ]
...
def evaluate_candidate(
    ...
    rich_specs: dict[str, stress.StressCandidate],
    X_by_view: dict[str, np.ndarray],
    y: np.ndarray,
    splits: list[tuple[np.ndarray, np.ndarray]],
) -> tuple[list[dict], list[dict]]:
    print(f"\nRich contact candidate: {spec.name} train_views={spec.train_views}", flush=True)
    ...
    for fold_id, (train_idx, val_idx) in enumerate(splits, start=1):
        ...
        artifact = stress.fit_stress_candidate(spec, base_specs, rich_specs, X_by_view, y, train_idx)
    ...
    for variant_name, bias, scores in contact.bias_variants(oof_by_view, y):
        ...
        row = base.make_report_row(
            ...
            split_name="rich_contact_stress_oof_cv",
            y_true=y,
            y_pred=clean_pred,
        ...
        row.update(
            {
                "feature_family": rich.FEATURE_FAMILY,
                "candidate": spec.name,
...
def main() -> None:
    ...
    run_slug = "audio_rich_contact_stress_cv_select"
    ...
    print("RICH_FEATURE_CACHE_DIR =", args.rich_feature_cache_dir.resolve())
    print("FEATURE_FAMILY         =", rich.FEATURE_FAMILY)
    print("Selection              = train-only rich audio contact-aware stress-CV")
    ...
    timings = {}
    for view in STRESS_VIEWS:
        payload, timing = rich.build_or_load_feature_cache(
            train_df,
            split_name="hand_train_full",
            view=view,
            feature_dir=args.rich_feature_cache_dir,
    ...
    split_path = split_dir / "hand_train_full_rich_contact_stress_cv_folds.csv"
    train_df.assign(cv_fold=fold_assignment).to_csv(split_path, index=False)

    split_summary = {
        "protocol": "audio-only rich contact-aware stress-CV; robot/test after lock",
    ...
    rich_specs = make_rich_contact_candidates()
    leaderboard_rows = []
    fold_rows = []
    for spec in rich_specs.values():
    ...
    leaderboard_path = report_dir / f"{run_slug}_oof_rich_contact_leaderboard.csv"
    ...
    selected_spec = rich_specs[str(selected["candidate"])]
    ...
    test_payload, test_timing = rich.build_or_load_feature_cache(
        test_df,
    ...
    final_row.update(
        {
            "selected_by": "hand_default_audio_only_rich_contact_stress_cv",
            "feature_family": rich.FEATURE_FAMILY,
    ...
    joblib.dump(
        {
            "protocol": "audio_only_rich_contact_stress_cv_select_no_test_until_final",
            "feature_family": rich.FEATURE_FAMILY,
    ...
    protocol_summary = {
        ...
        "rich_feature_cache_dir": str(args.rich_feature_cache_dir.resolve()),
        "selection_summary": selection_summary,
        "final_test_report": final_row,
    ...
    print("\nFinal robot/test result after frozen rich contact-aware audio selection:")
    print(
