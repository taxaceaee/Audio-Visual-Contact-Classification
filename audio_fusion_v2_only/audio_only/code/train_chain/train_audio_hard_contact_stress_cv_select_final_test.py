...
import train_audio_rich_stress_cv_select_final_test as rich
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Audio-only total240 with a harder train-only robot_hard stress view. "
            "Selection uses only hand/default; robot/test is loaded after lock."
        )
    )
...
def cache_paths(feature_dir: Path, view: str) -> dict[str, Path]:
...
def build_or_load_robot_hard_cache(
    frame: pd.DataFrame,
    hard_feature_dir: Path,
    force_rebuild: bool,
    n_jobs: int,
) -> tuple[dict[str, np.ndarray], dict[str, float | bool]]:
    view = "robot_hard"
    paths = cache_paths(hard_feature_dir, view)
    ...
    if ready and not force_rebuild:
        ...
        if valid:
            ...
            print(f"Loaded hard stress cache {view}: {payload['X'].shape} in {load_time:.3f}s")
            return payload, {
                "cache_hit": True,
        ...
        print("Hard stress cache metadata mismatch; rebuilding.")
    ...
    def extract_one(audio_path: str, label: int) -> tuple[np.ndarray, int, str]:
        ...
        stressed = rich.apply_audio_view(signal, view=view, key=str(audio_path), sr=sr)
        return stress.extract_features_from_signal(stressed, sr=sr), int(label), str(audio_path)
    ...
    if n_jobs == 1:
        results = [
            ...
            for audio_path, label in tqdm(jobs, total=len(jobs), desc="Extract total240/robot_hard")
        ]
    else:
        print(f"Parallel extract total240/robot_hard: n_jobs={n_jobs} samples={len(jobs)}", flush=True)
    ...
    if payload["X"].shape != (len(frame), base.EXPECTED_DIM):
        raise AssertionError(f"robot_hard: expected {(len(frame), base.EXPECTED_DIM)}, got {payload['X'].shape}")
    ...
    print(f"Saved hard stress cache {view}: {payload['X'].shape} in {extraction_time:.2f}s")
    return payload, {
        "cache_hit": False,
...
def make_hard_candidates() -> dict[str, stress.StressCandidate]:
    specs = [
        stress.StressCandidate("hard_contact_hgb_default__all_aug", "direct_hgb_default", "single", STRESS_VIEWS),
        stress.StressCandidate("hard_contact_hgb_regularized__all_aug", "direct_hgb_regularized", "single", STRESS_VIEWS),
        stress.StressCandidate("hard_contact_lgbm__all_aug", "direct_lightgbm_regularized", "single", STRESS_VIEWS),
        stress.StressCandidate("hard_contact_hier_extra_hgb__all_aug", "hier_extra_hgb", "single", STRESS_VIEWS),
        stress.StressCandidate("hard_contact_hier_hgb_lgbm__all_aug", "hier_hgb_lightgbm", "single", STRESS_VIEWS),
    ]
...
def bias_variants(proba_by_view: dict[str, np.ndarray], y_true: np.ndarray) -> list[tuple[str, np.ndarray, dict[str, float]]]:
...
def evaluate_candidate(
    ...
    hard_specs: dict[str, stress.StressCandidate],
    X_by_view: dict[str, np.ndarray],
    y: np.ndarray,
    splits: list[tuple[np.ndarray, np.ndarray]],
) -> tuple[list[dict], list[dict]]:
    print(f"\nHard contact candidate: {spec.name} train_views={spec.train_views}", flush=True)
    ...
    for fold_id, (train_idx, val_idx) in enumerate(splits, start=1):
        start = time.perf_counter()
        artifact = stress.fit_stress_candidate(spec, base_specs, hard_specs, X_by_view, y, train_idx)
    ...
    for variant_name, bias, scores in bias_variants(oof_by_view, y):
        ...
        row = base.make_report_row(
            ...
            split_name="hard_contact_stress_oof_cv",
            y_true=y,
            y_pred=clean_pred,
...
def main() -> None:
    ...
    run_slug = "audio_hard_contact_stress_cv_select"
    run_dir = args.output / "audio_feature_benchmarks" / run_slug
    report_dir = run_dir / "reports"
    model_dir = run_dir / "models"
    split_dir = run_dir / "splits"
    hard_feature_dir = run_dir / "stress_features"
    for directory in [run_dir, report_dir, model_dir, split_dir, hard_feature_dir]:
        directory.mkdir(parents=True, exist_ok=True)

    print("ROOT_PATH               =", root_path.resolve())
    print("RUN_DIR                 =", run_dir.resolve())
    print("CLEAN_FEATURE_CACHE_DIR =", args.clean_feature_cache_dir.resolve())
    print("BASE_STRESS_FEATURE_DIR =", args.base_stress_feature_dir.resolve())
    print("HARD_FEATURE_DIR        =", hard_feature_dir.resolve())
    print("Selection               = train-only 4-view hard contact-aware stress-CV")
    ...
    for view in ("robot_mix", "bandlimit"):
        payload, view_timing = stress.build_or_load_stress_cache(
            train_df,
            view=view,
            stress_feature_dir=args.base_stress_feature_dir,
            force_rebuild=False,
        )
        payloads[view] = payload
        timing[view] = view_timing
    hard_payload, hard_timing = build_or_load_robot_hard_cache(
        train_df,
        hard_feature_dir=hard_feature_dir,
        force_rebuild=args.force_rebuild_hard,
        n_jobs=args.n_jobs,
    )
    payloads["robot_hard"] = hard_payload
    timing["robot_hard"] = hard_timing
    ...
    split_path = split_dir / "hand_train_full_hard_contact_stress_cv_folds.csv"
    train_df.assign(cv_fold=fold_assignment).to_csv(split_path, index=False)

    split_summary = {
        "protocol": "audio-only total240 4-view hard contact-aware stress-CV; robot/test after lock",
    ...
    hard_specs = make_hard_candidates()
    leaderboard_rows = []
    fold_rows = []
    for spec in hard_specs.values():
    ...
    leaderboard_path = report_dir / f"{run_slug}_oof_hard_contact_leaderboard.csv"
    ...
    selected_spec = hard_specs[str(selected["candidate"])]
    ...
    final_row.update(
        {
            "selected_by": "hand_default_audio_only_hard_contact_stress_cv",
    ...
    joblib.dump(
        {
            "protocol": "audio_only_hard_contact_stress_cv_select_no_test_until_final",
            "feature_set": base.FEATURE_SET,
    ...
    protocol_summary = {
        ...
        "base_stress_feature_dir": str(args.base_stress_feature_dir.resolve()),
        "hard_feature_dir": str(hard_feature_dir.resolve()),
        "selection_summary": selection_summary,
        "final_test_report": final_row,
    ...
    print("\nFinal robot/test result after frozen hard contact-aware audio selection:")
    print(
