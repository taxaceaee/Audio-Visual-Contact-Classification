...
def main():
 report=OUT/'audio_feature_benchmarks/exact_anchor_spatial_image_group_selection'; report.mkdir(parents=True,exist_ok=True); audio_base.configure_feature_set('total240')
 ...
 high=group_audio.load_highsr_oof(OUT); pair=specimen.load_pairwise_oof(OUT/'audio_feature_benchmarks/audio_log_consensus_pair_blend_select/oof_sources/pairwise_selected_clean_oof_proba.npy'); pa=lift.anchor_lift_proba(af,high,pair); X=np.load(OUT/'audio_feature_benchmarks/image_spatial_v2_features/hand_train_full/X.npy').astype(np.float32)
 specs={'log':Pipeline([('scale',StandardScaler()),('model',LogisticRegression(C=.03,class_weight='balanced',max_iter=900,random_state=42))]),'extra':ExtraTreesClassifier(n_estimators=300,min_samples_leaf=4,max_features='sqrt',class_weight='balanced',random_state=42,n_jobs=8)}; rows=[]
 for name,s in specs.items():
  pi=np.zeros((len(y),4))
  for tr,va in folds: pi[va]=aligned(clone(s).fit(X[tr],y[tr]),X[va])
  for iw in np.linspace(0,.5,21):
   p=suite.normalize(np.exp((1-iw)*np.log(suite.normalize(pa))+iw*np.log(suite.normalize(pi)))); fs=[f1_score(y[va],p[va].argmax(1),average='macro',zero_division=0) for _,va in folds]; rows.append({'image_model':name,'image_weight':float(iw),'mean_fold_macro_f1':float(np.mean(fs)),'worst_fold_macro_f1':float(np.min(fs)),'std_fold_macro_f1':float(np.std(fs))})
 lb=pd.DataFrame(rows).sort_values(['worst_fold_macro_f1','mean_fold_macro_f1'],ascending=False).reset_index(drop=True); lb.to_csv(report/'hand_exact_anchor_spatial_leaderboard.csv',index=False); best=lb.iloc[0].to_dict(); lock={'protocol':'exact_anchor_spatial_image_specimen_group_hand_only','group_column':'specimen_group','n_folds':5,'test_loaded':False,'selected_candidate':best}; (report/'selection_lock.json').write_text(json.dumps(lock,indent=2,default=float)); print(json.dumps(lock,indent=2,default=float))
