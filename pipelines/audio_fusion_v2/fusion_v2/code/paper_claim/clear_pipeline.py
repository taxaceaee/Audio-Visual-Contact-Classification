"""Portable clear multimodal claim pipeline (equiv. sealed paper_claim_v2).

All data roots resolve relative to the release folder (or TREE_BUNDLE_ROOT).
...
def _release_root() -> Path:
    # .../fusion_v2/code/paper_claim/clear_pipeline.py -> bundle root
...
def bundle_paths(root: Path | None = None) -> dict[str, Path]:
    ...
    root = Path(os.environ.get("TREE_BUNDLE_ROOT", root or _release_root())).resolve()
    data = root / "fusion_v2" / "data"
    return {
        "root": root,
