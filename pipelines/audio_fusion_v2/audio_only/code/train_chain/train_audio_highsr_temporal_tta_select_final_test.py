...
def main() -> None:
    ...
    # Persist the frozen per-view test probabilities as an audit/debug artifact.
    # The TTA selection remains hand-OOF-only; these arrays are written only
    # after the selection summary has been written and are never used to tune it.
    for view, proba in final_proba_by_view.items():
        np.save(report_dir / f"{run_slug}_{view}_test_proba.npy", proba)
