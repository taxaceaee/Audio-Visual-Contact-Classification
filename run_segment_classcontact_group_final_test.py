...
def main():
    lock=json.loads((OUT/'selection_lock.json').read_text()); assert not lock.get('test_loaded'); b=lock['selected_candidate']; ts=np.array([float(b['leaf_contact_threshold']),float(b['trunk_contact_threshold']),float(b['twig_contact_threshold'])]); audio_base.configure_feature_set('total240')
    ...
    tfr=suite.load_manifest(ROOT/'audio_visual_dataset_robo_default/dataset.csv',ROOT/'audio_visual_dataset_robo_default','robot_test'); yt=tfr.y.to_numpy(np.int64); xt=np.load(IMG/'robot_test/X.npy').astype(np.float32); tu,tc,tx,ty=pool(tfr.audio_file,xt,yt); img=model.predict_proba(tx)
    audio=pd.read_csv('outputs/audio_feature_benchmarks/audio_lift_source_blend_select/reports/audio_lift_source_blend_select_final_test_predictions.csv'); raw=pd.read_csv(ROOT/'audio_visual_dataset_robo_default/dataset.csv'); assert np.array_equal(audio.audio_file.astype(str).to_numpy(),raw.audio_file.astype(str).to_numpy()); proba=suite.normalize(audio[suite.PROBA_COLUMNS].to_numpy(np.float64)); pc,lg=avr.avr_inputs(proba); sk=seg(tfr.audio_file).to_numpy(); spc=np.array([pc[sk==k].mean() for k in tu]); sa=np.vstack([suite.normalize(np.exp(lg[sk==k].mean(0,keepdims=True)))[0] for k in tu]); mat=suite.normalize(sa*np.array([1.0,.6,0.0])+img*np.array([0.0,.4,1.0])); cls=mat.argmax(1)+1; pred_seg=np.where(spc>=ts[cls-1],cls,0); order={k:i for i,k in enumerate(tu)}; pred=np.array([pred_seg[order[k]] for k in sk]); by=(yt>0).astype(int); bp=(pred>0).astype(int)
    ...
    (OUT/'segment_classcontact_final_test_metrics.json').write_text(json.dumps(result,indent=2,default=float)); print(json.dumps(result,indent=2,default=float))
